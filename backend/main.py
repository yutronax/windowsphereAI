...
def get_session_for_excel_read(payload: ExcelReadRequest) -> SessionContext:
    """`get_session_for_search` ile AYNI mantık, `ExcelReadRequest` şeması
    için - Saga #326."""
    session = _sessions.get(payload.sessionId)
    if session is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Oturum bulunamadı")
...
def excel_read_endpoint(
    payload: ExcelReadRequest,
    session: SessionContext = Depends(get_session_for_excel_read),
) -> ExcelReadResponse:
    """Saga #326: Excel okuma endpoint'i - salt-okunur senkron sorgu,
    `search_endpoint`'in (Saga #313) AYNI session/allowed_root doğrulama
    deseni (`get_session_for_excel_read` ~ `get_session_for_search`,
    `allowed_root.is_dir()` 410 kontrolü)."""
    allowed_root = Path(session.selectedFolder)
    if not allowed_root.is_dir():
        raise HTTPException(
            status_code=status.HTTP_410_GONE,
    ...
    try:
        values = excel_rows.read_excel_range(source_path, payload.range)
    except ValueError as exc:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=f"Geçersiz range: '{payload.range}' ({exc})",
        )
    except Exception as exc:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=f"Excel dosyası okunamıyor: '{payload.filename}' ({exc})",
        )
